...
#### OVERVIEW ###########
# A. Install and Load Libraried (not actualy needed for this script)
...
# INPUTS: Load DTM Raster ------------
  ...
  # DTM <- raster(paste(DTMpath, "\\", DTM, sep = "")) # read with GDAL, and Write with GDAL works well. 
...
# OUTPUTS: ------------------------------------------------------------
  ...
  ifelse(!dir.exists(file.path(tmpOut)),               #if 'TS' Does not Exists
          dir.create(file.path(tmpOut)), FALSE)        #create TS
