...
pts = pts.0 %>% dplyr::select(c(Longitude, Latitude,GlobalID,Biogeoclimatic.Unit,Site.Group_5m,Site.Series.Map.Unit_5m,MAP_LABEL,BGC_test,Crew,
...
file1 = att.files[2] # choose the scale of interest (change the no to select a different scale)
scale.analysis = gsub("*D_|_pts.*","", file1) 
scale.analysis = gsub(".*D_|_pts.*","", file1) 
...
qdatafn <- qdatafn %>%
  mutate (SiteSeries = Site.Series.Map.Unit_5m, ID = seq(1,length(qdatafn$GlobalID),1)) %>%
  select(-c(GlobalID))

# If your subset contains 
...
pred <- read.table(paste(output.folder,"/",MODELfn,"_pred.csv",sep = ""),
...
dline = c(MODELfn,no.params,scale,CMX.PCC,Ave.om,Ave.com,kap,kap.sd)
...
folder = paste(layer.folder,"Dec_North_25m/layers",sep = "/") # create the filepath where the layers are stored (use the same scale as points extracted from )
list.files(folder)

rastLUTfn <- "ModelMapData_LUT.csv"
