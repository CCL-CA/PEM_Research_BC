...
#m.to.run <- 2; m.no <- m.to.run # tester
...
if(length(m.to.run)<2) { 
  m.no <- m.to.run } else { 
    ...
    for (i in 1:length(m.to.run)) { 
      ...
      pts.0 = read.csv(paste(field.data.folder,"/",pts.file,sep = ''),stringsAsFactors = FALSE)     #head(pts.0) ; length(pts.0$Longitude) # error check
      
      ## generate blank tables for each level of response to aggregate resulst summary 
      ## NOTE : only need to do this ONCE! before running models; 
      #SS <- as.data.frame(unique(pts.0[["Site.Series.Map.Unit_5m"]])); colnames(SS)<-"Site.Series.Map.Unit_5m"
      #write.csv(SS, file= paste(model.folder.out,"/","Site.Series.Map.Unit_5m_prop.correct.csv",sep=""))
      ## repeat this for other response types: 
      ...
      map.unit.correct <- rownames_to_column(map.unit.correct, response.name)
      
      map.unit.correct <- map.unit.correct %>% mutate(prop.correct = 1- error) %>% select(paste(response.name), prop.correct)
      ...
      prop.cor<- prop.cor %>% select(-c(X))
      write.csv(prop.cor,paste(model.folder,"/",response.name,"_prop.correct.csv",sep = "")) 
